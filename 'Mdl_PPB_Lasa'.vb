    ...
    Dim Percentual As Double, unidade As Double, Etapa As String
        ...
        Call AtualizarStatus("1/3", 1, 0)
            ...
            Call AbrirBDPPB
                ...
                & " FROM Tbl_PPB INNER JOIN TBL_PEDIDO_LASA ON (Tbl_PPB.Numero_Nota = TBL_PEDIDO_LASA.[Nº Nota Fiscal]) AND (Tbl_PPB.COD_OPERACAO = TBL_PEDIDO_LASA.Cód_Oper) AND (Tbl_PPB.Convenio_Ancora = TBL_PEDIDO_LASA.Convênio) AND (Tbl_PPB.Agencia_Ancora = TBL_PEDIDO_LASA.Agência) WHERE (((Tbl_PPB.Agencia_Ancora)=57) AND ((Tbl_PPB.Convenio_Ancora)='008500000024') AND ((Tbl_PPB.Data_Op)>=#" & DataInic & "# And (Tbl_PPB.Data_Op)<=#" & DataFin & "#));")
            
                'Calcular PPB
                Set TbDados = BDPPB.OpenRecordset("SELECT Tbl_PPB_FINAL.Agencia_Ancora, Tbl_PPB_FINAL.CHAVE, Tbl_PPB_FINAL.Convenio_Ancora, Tbl_PPB_FINAL.Data_Op, Tbl_PPB_FINAL.Prazo, Tbl_PPB_FINAL.Valor_Nota, Tbl_PPB_FINAL.Valor_Juros, Tbl_PPB_FINAL.Taxa, Tbl_PPB_FINAL.Custo, Tbl_PPB_FINAL.Valor_Banco, Tbl_PPB_FINAL.PPB_Bruto FROM Tbl_PPB_FINAL WHERE (((Tbl_PPB_FINAL.Agencia_Ancora)='57') AND ((Tbl_PPB_FINAL.Convenio_Ancora)='008500000024') AND ((Tbl_PPB_FINAL.Data_Op)>=#" & DataInic & "# And (Tbl_PPB_FINAL.Data_Op)<=#" & DataFin & "#));", dbOpenDynaset)
    ...
    Dim Data(500) As Date, DataInicioPlan As Date, DiasCalc(500) As Integer
        ...
        Call AbrirBDPPB
            ...
            Call AtualizarStatus("2/3", 1, 0)
                ...
                DataInic = Format(DataInicio, "m/dd/YYYY")
                DataFin = Format(DataFinal, "mm/dd/YYYY")
                    ...
                    DiferimentoDiario = (TbDados!PPB_Bruto / TbDados!Prazo): DataVencimento = TbDados!Data_Venc: DataOperação = TbDados!Data_op
                        ...
                        If ValorDif(1) > 0 Then
                            ...
                            TbDiferimento.AddNew
                                TbDiferimento!Agencia_Ancora = "57"
                                TbDiferimento!Convenio_Ancora = "008500000024"
                                TbDiferimento!Nome_Ancora = "LASA"
                ...
                Caminho = "\\saont46\apps2\Confirming\PROJETORELATORIOS\MASCARAS\XLSX\"
                   ...
                   Set ObjPlan1Excel = ObjExcel.Worksheets("Operações")
                    ...
                    linha = 7: MesData = UCase(MonthName(Month(DataInicio)))
                       ...
                       ObjPlan1Excel.Range("B2") = "Cliente:  LASA"
                    ...
                    TbDados.MoveLast: TbLinha = TbDados.RecordCount: UltimaLinha = TbLinha + linha
                      ...
                      'ObjPlan1Excel.Rows(UltimaLinha & ":8550").Delete Shift:=xlUp
                    TbDados.Close
                    ...
                    Set ObjPlan2Excel = ObjExcel.Worksheets("Alocação PPB")
                        ...
                        ObjPlan2Excel.Range("F3") = Format(DataInicio, "MM/DD/YYYY"): ObjPlan2Excel.Range("N3") = Resultado(2): ObjPlan2Excel.Range("R3") = Resultado(3): ObjPlan2Excel.Range("V3") = Resultado(4): ObjPlan2Excel.Range("Z3") = Resultado(5): ObjPlan2Excel.Range("AD3") = Resultado(6): ObjPlan2Excel.Range("AH3") = Resultado(7): ObjPlan2Excel.Range("AL3") = Resultado(8): ObjPlan2Excel.Range("AP3") = Resultado(9): ObjPlan2Excel.Range("AT3") = Resultado(10): ObjPlan2Excel.Range("AX3") = Resultado(11): ObjPlan2Excel.Range("BB3") = Resultado(12)
                  ...
                  Set TbDados = BDPPB.OpenRecordset("SELECT Tbl_PPB_FINAL.Data_Op, Tbl_PPB_FINAL.Nome_Fornecedor, Tbl_PPB_FINAL.CNPJ_Fornecedor, Tbl_PPB_FINAL.Numero_Nota, Tbl_PPB_FINAL.Data_Venc, Tbl_PPB_FINAL.Valor_Nota, Tbl_PPB_FINAL.Taxa, Tbl_PPB_FINAL.Prazo, Tbl_PPB_FINAL.Valor_Juros, Tbl_PPB_FINAL.Custo, Tbl_PPB_FINAL.Valor_Banco, Tbl_PPB_FINAL.PPB_Bruto FROM Tbl_PPB_FINAL WHERE (((Tbl_PPB_FINAL.Data_op) >= #" & DataInic & "# And (Tbl_PPB_FINAL.Data_op) <= #" & DataFin & "#) And ((Tbl_PPB_FINAL.Agencia_Ancora) = '57') And ((Tbl_PPB_FINAL.Convenio_Ancora) = '008500000024')) ORDER BY Tbl_PPB_FINAL.Data_Op;", dbOpenDynaset)
                    ...
                    If TbDados.EOF = False Then
                       ...
                       TbDados.MoveLast: Quantidade = TbDados.RecordCount: TbDados.MoveFirst
                       Contador = 0: unidade = Round((8200 / Quantidade), 2): Etapa = "1/4"
                      ...
                      UltimaLinha = TbLinha + 4
                        ...
                        ObjPlan2Excel.Columns.AutoFit
                         ...
                         Nome = "PPB LASA - " & MesData & " - " & Right(Date, 4)
