...
Function ImportarCustoCarajasExcel(Caminho As String)

    Dim ObjExcelCusto As Object, TbCustoCarajas As Recordset
        ...
        Set ObjExcelCusto = ObjExcelCusto.Worksheets("Plan1")
            ...
            Do While True
                ...
                Set TbCustoCarajas = BDPPB.OpenRecordset("Tbl_Custo_Carajas", dbOpenDynaset)

                    TbCustoCarajas.AddNew
                        TbCustoCarajas!Agencia_Ancora = "4657"
                        TbCustoCarajas!Convenio_Ancora = "008500000025"
                        TbCustoCarajas!Data = Trim(ObjExcelCusto.Cells(LinhaCusto, 1))
                        TbCustoCarajas!Prazo = Trim(ObjExcelCusto.Cells(LinhaCusto, 2))
                        TbCustoCarajas!Custo = Trim(ObjExcelCusto.Cells(LinhaCusto, 3))
                        TbCustoCarajas!Prazo_2 = Trim(ObjExcelCusto.Cells(LinhaCusto, 4))
                        TbCustoCarajas!Vencimento = Trim(ObjExcelCusto.Cells(LinhaCusto, 5))
                    TbCustoCarajas.Update
...
Sub AdicionarCustosCarajas()

    Dim TbDados As Recordset, CustoCarajas As QueryDef, sFname As String
        ...
        Call AbrirBDPPB
          ...
          DataFin = Format(DataFinal, "mm/dd/YYYY")
           ...
           Call AtualizarStatus("1/5", 1, 0)
            ...
            Set CustoCarajas = BDPPB.CreateQueryDef("CustoCarajas", "SELECT Tbl_Custo_Carajas.Agencia_Ancora, Tbl_Custo_Carajas.Convenio_Ancora, Tbl_Custo_Carajas.Data FROM Tbl_Custo_Carajas GROUP BY Tbl_Custo_Carajas.Agencia_Ancora, Tbl_Custo_Carajas.Convenio_Ancora, Tbl_Custo_Carajas.Data HAVING (((Tbl_Custo_Carajas.Agencia_Ancora)='4657') AND ((Tbl_Custo_Carajas.Convenio_Ancora)='008500000025') AND ((Tbl_Custo_Carajas.Data)>=#" & DataInic & "# And (Tbl_Custo_Carajas.Data)<=#" & DataFin & "#));")

                BDPPB.Containers.Refresh

            Set TbDados = BDPPB.OpenRecordset("SELECT CustoCarajas.Data, TblCalendario.Data_dia, TblCalendario.Tipo FROM CustoCarajas RIGHT JOIN TblCalendario ON CustoCarajas.Data = TblCalendario.Data_dia WHERE (((CustoCarajas.Data) Is Null) AND ((TblCalendario.Data_dia)>=#" & DataInic & "# And (TblCalendario.Data_dia)<Date()) AND ((TblCalendario.Tipo)='Util'));", dbOpenDynaset)
                ...
                If TbDados.EOF = False Then
                    ...
                    Contador = 0: unidade = Round((8200 / Qntd), 2): Etapa = "1/5"
                        ...
                        Debug.Print TbDados!Data_dia
                            sFname = "\\saont46\apps2\Confirming\PlanCarajas\Planilhas Custo Carajas\Planilha Carajas " & Format(TbDados!Data_dia, "ddmmyyyy") & ".xlsx"
                                If (Dir(sFname) <> "") Then
                                    Call ImportarCustoCarajasExcel(sFname)
                                End If
                            TbDados.MoveNext
                           Contador = Contador + 1
                        ...
                        Loop
                    BDPPB.Execute ("Drop Table CustoCarajas")
                  BDPPB.Close
                End If
End Sub
...
Sub GerarPPBCarajas()
    ...
    Dim Percentual As Double, unidade As Double, Etapa As String
        ...
        Call AtualizarStatus("3/5", 1, 0)
            ...
            & " FROM Tbl_PPB_FINAL WHERE ((Tbl_PPB_FINAL.Agencia_Ancora='4657' AND Tbl_PPB_FINAL.Convenio_Ancora='008500000025') AND ((Tbl_PPB_FINAL.Data_Venc)>=#" & DataInic & "# And (Tbl_PPB_FINAL.Data_Venc)<=#" & DataFin & "#));", dbOpenDynaset)
            ...
            BDPPB.Execute ("INSERT INTO Tbl_PPB_FINAL ( Nome_Ancora, CNPJ_Ancora, Agencia_Ancora, Convenio_Ancora, Data_Op, Nome_Fornecedor, CNPJ_Fornecedor, COD_OPERACAO, Numero_Nota, Data_Venc, Valor_Nota, Taxa, Prazo, Valor_Juros, Custo, Valor_Banco, PPB_Bruto, CHAVE ) SELECT Tbl_PPB.Nome_Ancora, Tbl_PPB.CNPJ_Ancora, Tbl_PPB.Agencia_Ancora, Tbl_PPB.Convenio_Ancora, Tbl_PPB.Data_Op, Tbl_PPB.Nome_Fornecedor, Tbl_PPB.CNPJ_Fornecedor, Tbl_PPB.COD_OPERACAO, Tbl_PPB.Numero_Nota, Tbl_PPB.Data_Venc, Tbl_PPB.Valor_Nota, Tbl_PPB.Taxa, Tbl_PPB.Prazo, Tbl_PPB.Valor_Juros, Tbl_Custo_Carajas.Custo, Tbl_PPB.Valor_Banco, Tbl_PPB.PPB_Bruto, Tbl_PPB.CHAVE" _
            & " FROM Tbl_PPB INNER JOIN Tbl_Custo_Carajas ON (Tbl_PPB.Prazo = Tbl_Custo_Carajas.Prazo) AND (Tbl_PPB.Data_Op = Tbl_Custo_Carajas.Data) WHERE ((Tbl_PPB.Agencia_Ancora=4657 AND Tbl_PPB.Convenio_Ancora='008500000025') AND ((Tbl_PPB.Data_Op)>=#" & DataInic & "# And (Tbl_PPB.Data_Op)<=#" & DataFin & "#) AND ((Tbl_Custo_Carajas.Agencia_Ancora)='4657') AND ((Tbl_Custo_Carajas.Convenio_Ancora)='008500000025'));")
                        ...
                        DiferimentoDiario = (TbDados!PPB_Bruto / TbDados!Prazo): DataVencimento = TbDados!Data_Venc: DataOperação = TbDados!Data_op
                            ...
                            If ValorDif(1) > 0 Then
                                ...
                                TbDiferimento.AddNew
                                    ...
                                    TbDiferimento!Nome_Ancora = "Carajas"
...
Sub DiferimentoExcelCarajas()
    ...
    Dim DataDaPesquisa, Resultado(24), DataVencimento, DataOperação, Databasemais As Date
        ...
        Call AbrirBDPPB
                ...
                DataInic = Format(DataInicio, "mm/dd/yyyy")
                        ...
                        If TbDados.EOF = False Then
                            ...
                            MesData = UCase(MonthName(Month(DataFinal)))
                                ...
                                ObjPlan1Excel.Range("B2") = "Cliente:  CARAJAS MATERIAL DE CONSTRUCAO"
            ...
            Set TbDados = BDPPB.OpenRecordset("SELECT Tbl_PPB_FINAL.Data_Op, Tbl_PPB_FINAL.Nome_Fornecedor, Tbl_PPB_FINAL.CNPJ_Fornecedor, Tbl_PPB_FINAL.Numero_Nota, Tbl_PPB_FINAL.Data_Venc, Tbl_PPB_FINAL.Valor_Nota, Tbl_PPB_FINAL.Taxa, Tbl_PPB_FINAL.Prazo, Tbl_PPB_FINAL.Valor_Juros, Tbl_PPB_FINAL.Custo, Tbl_PPB_FINAL.Valor_Banco, Tbl_PPB_FINAL.PPB_Bruto FROM Tbl_PPB_FINAL WHERE ((Tbl_PPB_FINAL.Agencia_Ancora = '4657' And Tbl_PPB_FINAL.Convenio_Ancora = '008500000025') And ((Tbl_PPB_FINAL.Data_Venc) >= #" & DataInic & "# And (Tbl_PPB_FINAL.Data_Venc) <= #" & DataFin & "#)) ORDER BY Tbl_PPB_FINAL.Data_Op;", dbOpenDynaset)
        ...
        Set TbTituloColuna = BDPPB.OpenRecordset("SELECT Tbl_Diferimento.Data_Dif FROM Tbl_Diferimento INNER JOIN Tbl_PPB_FINAL ON Tbl_Diferimento.CHAVE = Tbl_PPB_FINAL.CHAVE WHERE ( ( (Tbl_Diferimento.Agencia_Ancora = '4657') And (Tbl_Diferimento.Convenio_Ancora = '008500000025') And (Tbl_PPB_FINAL.Data_Venc >= #" & DataFin & "#) ) ) GROUP BY Tbl_Diferimento.Data_Dif ORDER BY Tbl_Diferimento.Data_Dif;", dbOpenDynaset)
        ...
        & " FROM Tbl_Diferimento INNER JOIN Tbl_PPB_FINAL ON Tbl_Diferimento.CHAVE = Tbl_PPB_FINAL.CHAVE WHERE (((Tbl_PPB_FINAL.Data_Venc) >= #" & DataFin & "#) And ((Tbl_Diferimento.Agencia_Ancora) = '4657') And ((Tbl_Diferimento.Convenio_Ancora) = '008500000025')) GROUP BY Tbl_PPB_FINAL.Data_Op, Tbl_PPB_FINAL.Prazo, Tbl_PPB_FINAL.Data_Venc, Tbl_PPB_FINAL.PPB_Bruto, ([Tbl_PPB_FINAL]![PPB_Bruto]/[Tbl_PPB_FINAL]![Prazo]), Tbl_PPB_FINAL.Numero_Nota ORDER BY Tbl_PPB_FINAL.Data_Op PIVOT Tbl_Diferimento.Data_Dif;", dbOpenDynaset) '
        ...
        Nome = "PPB Carajas - " & MesData & " - " & Right(Date, 4)
